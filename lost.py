...
class Link(object):
    ...
    def __init__(self,url,text,regex = None):
        self.url = url
        self.text = text
        self.regex = regex
def get_cid(s):
    import re
    res = re.search(s.regex,s.url)
    return int(res.groupdict()['cid'])
...
class Lost(BaseSpider):
    ...
    def __init__(self,*args, **kwargs):
        ...
        if self.config in type:
            ...
            self.content = arg.get('content',None)
            self.filters = arg.get('filter',None)
            self.xpath_book = arg.get('book',None)
            self.author = arg.get('author',None)
            self.regex = arg.get('regex',None)
            log.msg(args)
        else:
            log.msg(u"配置文件不正确")
            return
    ...
    def parse(self, response):
        ...
        if self.chapter_list:
            ...
            links = [Link(url=l.url,text=l.text,regex=self.regex) for l in links]
            log.msg(u"成功获取章节列表")
            if self.regex:
                log.msg("对章节列表进行排序")
